...
test('authoritative enemy spatial grid reuses maps, cells and query scratch across ticks',()=>{const {createSpatialGrid}=require('../server'),first=[{id:'a',x:139,y:70,r:13},{id:'b',x:154,y:70,r:13}],grid=createSpatialGrid(first,140),cells=grid.cells,index=grid.indexByEntity,projectile=grid.projectileQuery,splash=grid.splashQuery,cellArrays=[...grid.cells.values()];const second=Array.from({length:80},(_,id)=>({id:`n${id}`,x:id*10,y:140,r:13}));assert.equal(createSpatialGrid(second,140,null,grid),grid);assert.equal(grid.cells,cells);assert.equal(grid.indexByEntity,index);assert.equal(grid.projectileQuery,projectile);assert.equal(grid.splashQuery,splash);assert.ok([...grid.cells.values()].some(cell=>cellArrays.includes(cell)));assert.ok(grid.projectileQuery.seen.length>=second.length)});
