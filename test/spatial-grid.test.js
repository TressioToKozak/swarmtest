...
test('steady-state rebuild reuses cell arrays and stale cells are pruned',()=>{const grid=new SpatialGrid(128),entities=[{id:1,x:64,y:64,r:18},{id:2,x:190,y:64,r:18}];grid.rebuild(entities);const created=grid.cellArraysCreated,cells=[...grid.cells.values()].map(cell=>cell.indices);for(let frame=0;frame<60;frame++)grid.rebuild(entities);assert.equal(grid.cellArraysCreated,created);assert.deepEqual([...grid.cells.values()].map(cell=>cell.indices),cells);entities[0].x=5000;entities[1].x=5200;for(let frame=0;frame<241;frame++)grid.rebuild(entities);assert.ok(grid.cells.size<10);assert.ok(grid.activeCellCount>0)});
...
test('browser integration uses the broad phase while retaining the exact narrow phase',()=>{
  ...
  assert.match(html,/spatial-grid\.js\?v=2/);
