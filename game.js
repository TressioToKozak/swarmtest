...
const ui = Object.fromEntries(['timer','kills','level','healthText','healthBar','xpLevel','xpText','xpBar','slots','cards','startModal','instructionsModal','statsModal','pauseModal','levelModal','endModal','newLevel','endStats','bestTime','totalTime','bestKills','totalKills'].map(id=>[id,document.getElementById(id)]));
let W=0,H=0,dpr=1,last=0,running=false,paused=true,elapsed=0,spawnClock=0,shotClock=0,screenShake=0,flowClock=0,statsClock=0;
...
function togglePause(){if(!running||!ui.levelModal.classList.contains('hidden')||!ui.endModal.classList.contains('hidden'))return;paused=!paused;ui.pauseModal.classList.toggle('hidden',!paused);keys.w=keys.a=keys.s=keys.d=false}
...
function spawnEnemy(){let x,y;for(let tries=0;tries<20;tries++){const a=Math.random()*Math.PI*2,d=Math.max(W,H)*(.58+Math.random()*.12);x=Math.max(35,Math.min(WORLD.w-35,player.x+Math.cos(a)*d));y=Math.max(35,Math.min(WORLD.h-35,player.y+Math.sin(a)*d));if(!blocked(x,y,28)&&dist({x,y},player)>350)break}if(blocked(x,y,28))return;const brute=Math.random()<Math.min(.08+elapsed/300,.28);enemies.push({x,y,r:brute?24:13,hp:brute?65+elapsed*.5:22+elapsed*.25,max:brute?65+elapsed*.5:22+elapsed*.25,speed:brute?48:72+Math.min(elapsed,150)*.25,brute,hit:0})}
...
function update(dt){elapsed+=dt;flowClock-=dt;if(flowClock<=0){rebuildFlow();flowClock=.4}statsClock+=dt;if(statsClock>=1){savedStats.totalTime+=Math.floor(statsClock);statsClock%=1;saveStats()}player.invuln=Math.max(0,player.invuln-dt);Object.values(skills).forEach(s=>s.cd=Math.max(0,s.cd-dt));let dx=(keys.d?1:0)-(keys.a?1:0),dy=(keys.s?1:0)-(keys.w?1:0),l=Math.hypot(dx,dy)||1,speed=player.speed*(1+.12*(levels.speed||0));const nextX=player.x+dx/l*speed*dt,nextY=player.y+dy/l*speed*dt;if(!blocked(nextX,player.y,player.r))player.x=nextX;if(!blocked(player.x,nextY,player.r))player.y=nextY;camera.x=Math.max(0,Math.min(WORLD.w-W,player.x-W/2));camera.y=Math.max(0,Math.min(WORLD.h-H,player.y-H/2));shotClock-=dt;if(shotClock<=0){shoot();shotClock=.62/Math.pow(1.18,levels.rapid||0)}spawnClock-=dt;if(spawnClock<=0){spawnEnemy();spawnClock=Math.max(.22,.82-elapsed*.004)}
  ...
  enemies.filter(e=>e.hp<=0).forEach(e=>{player.kills++;savedStats.totalKills++;saveStats();orbs.push({x:e.x,y:e.y,r:5,value:e.brute?6:3,vx:0,vy:0});burst(e.x,e.y,e.brute?'#b56cff':'#ff5577',14)});enemies=enemies.filter(e=>e.hp>0);
...
function drawEnemy(e){ctx.save();ctx.translate(e.x,e.y);const angle=Math.atan2(player.y-e.y,player.x-e.x);ctx.rotate(angle);const pulse=1+Math.sin(elapsed*7+e.x)*.04;ctx.scale(pulse,pulse);ctx.shadowBlur=e.hit?24:14;ctx.shadowColor=e.brute?'#a06cff':'#ff3e6d';ctx.fillStyle=e.hit?'#fff':e.brute?'#3e255e':'#411629';ctx.strokeStyle=e.brute?'#c781ff':'#ff557c';ctx.lineWidth=2;
  ctx.beginPath();ctx.moveTo(e.r*1.18,0);ctx.bezierCurveTo(e.r*.45,-e.r*.9,-e.r*.5,-e.r,-e.r*.85,-e.r*.35);ctx.lineTo(-e.r*1.25,-e.r*.7);ctx.lineTo(-e.r*.92,0);ctx.lineTo(-e.r*1.25,e.r*.7);ctx.bezierCurveTo(-e.r*.5,e.r,e.r*.45,e.r*.9,e.r*1.18,0);ctx.fill();ctx.stroke();
  ctx.fillStyle=e.brute?'#d4a0ff':'#ff9aae';ctx.shadowBlur=10;ctx.beginPath();ctx.ellipse(e.r*.35,-e.r*.22,e.r*.2,e.r*.11,-.25,0,7);ctx.ellipse(e.r*.35,e.r*.22,e.r*.2,e.r*.11,.25,0,7);ctx.fill();ctx.fillStyle='#090713';ctx.beginPath();ctx.arc(e.r*.48,0,e.r*.17,0,7);ctx.fill();ctx.restore();
  if(e.brute){ctx.fillStyle='#161427';ctx.fillRect(e.x-22,e.y-e.r-14,44,4);ctx.fillStyle='#a96cff';ctx.fillRect(e.x-22,e.y-e.r-14,44*e.hp/e.max,4)}
}
...
function draw(){ctx.save();if(screenShake)ctx.translate((Math.random()-.5)*screenShake,(Math.random()-.5)*screenShake);ctx.fillStyle='#070b15';ctx.fillRect(0,0,W,H);ctx.translate(-camera.x,-camera.y);
  ...
  bullets.forEach(b=>{ctx.shadowBlur=14;ctx.shadowColor='#51f6df';ctx.fillStyle='#bafff7';ctx.beginPath();ctx.arc(b.x,b.y,b.r,0,7);ctx.fill();ctx.shadowBlur=0});enemies.forEach(drawEnemy);
