...
let crates=[
  {x:1080,y:790},{x:1380,y:350},{x:1730,y:560},{x:2260,y:820},{x:2620,y:1260},{x:1960,y:1390},{x:1740,y:1850},{x:1010,y:1730},{x:420,y:1510},{x:620,y:730}
].map((c,i)=>({...c,id:i,hp:28,max:28,opened:false,hit:0}));
const ui = Object.fromEntries(['timer','kills','level','healthText','healthBar','xpLevel','xpText','xpBar','slots','cards','startModal','instructionsModal','statsModal','pauseModal','levelModal','endModal','newLevel','endStats','bestTime','totalTime','bestKills','totalKills','phaseLabel','waveBanner','waveCaption','waveTitle','charactersModal','mapsModal'].map(id=>[id,document.getElementById(id)]));
let W=0,H=0,dpr=1,last=0,running=false,paused=true,elapsed=0,spawnClock=0,shotClock=0,screenShake=0,flowClock=0,statsClock=0,saveClock=0,directorPhase=-1,waveNumber=0,waveRemaining=0,waveBreak=0,bannerTimer=0;
...
function saveGame(){
  if(!running)return;const state={version:2,savedAt:Date.now(),elapsed,spawnClock,shotClock,directorPhase,waveNumber,waveRemaining,waveBreak,player:{...player},skills:{q:{...skills.q},e:{...skills.e},r:{...skills.r}},levels:{...levels},enemies:enemies.slice(-450),bullets:bullets.slice(-250),enemyBullets:enemyBullets.slice(-250),orbs:orbs.slice(-350),loot,crates,rings,levelPending:!ui.levelModal.classList.contains('hidden')};
...
function updateDirector(dt){
  const d=directorState();if(d.id!==directorPhase)startDirectorPhase();if(bannerTimer>0){bannerTimer-=dt;if(bannerTimer<=0)ui.waveBanner.classList.add('hidden')}
...
function choose(id){levels[id]=(levels[id]||0)+1;if(id==='vital'){player.maxHp+=20;player.hp=Math.min(player.maxHp,player.hp+30)}ui.levelModal.classList.add('hidden');paused=false;renderSlots();saveGame()}
...
function drawSelectionPreviews(){
  const character=document.querySelector('#characterPreviewCanvas'),cc=character.getContext('2d');cc.clearRect(0,0,character.width,character.height);const cg=cc.createRadialGradient(250,115,10,250,115,180);cg.addColorStop(0,'#173e4b');cg.addColorStop(1,'#07101c');cc.fillStyle=cg;cc.fillRect(0,0,500,230);cc.save();cc.translate(250,115);cc.scale(3.2,3.2);cc.shadowBlur=18;cc.shadowColor='#51f6df';cc.fillStyle='#0b2630';cc.strokeStyle='#61fff0';cc.lineWidth=2;cc.beginPath();cc.moveTo(23,0);cc.lineTo(7,-12);cc.lineTo(-10,-15);cc.lineTo(-5,-6);cc.lineTo(-18,-4);cc.lineTo(-18,4);cc.lineTo(-5,6);cc.lineTo(-10,15);cc.lineTo(7,12);cc.closePath();cc.fill();cc.stroke();cc.fillStyle='#eaffff';cc.beginPath();cc.arc(-3,0,5,0,7);cc.fill();cc.restore();
  const map=document.querySelector('#mapPreviewCanvas'),mc=map.getContext('2d'),sx=500/WORLD.w,sy=230/WORLD.h;mc.fillStyle='#050a12';mc.fillRect(0,0,500,230);tracePolygon(mc,MAP_SHAPE.map(([x,y])=>[x*sx,y*sy]));mc.fillStyle='#163142';mc.fill();terrain.forEach(t=>{if(t.type==='water'){tracePolygon(mc,t.points.map(([x,y])=>[x*sx,y*sy]));mc.fillStyle='#0d6074';mc.fill()}else if(t.type==='wall'){mc.fillStyle='#778598';mc.fillRect(t.x*sx,t.y*sy,t.w*sx,t.h*sy)}else{mc.fillStyle='#3b4657';mc.beginPath();mc.arc(t.x*sx,t.y*sy,Math.max(3,t.r*sx),0,7);mc.fill()}});crates.forEach(c=>{mc.fillStyle='#d9a34d';mc.fillRect(c.x*sx-2,c.y*sy-2,4,4)})
}
