...
function enemyAt(x,y,type='melee'){
  const scale=1+elapsed*.009,stats={melee:{r:13,hp:22,speed:76},brute:{r:24,hp:70,speed:49},shooter:{r:16,hp:32,speed:62},charger:{r:19,hp:48,speed:67},exploder:{r:17,hp:35,speed:61},swarm:{r:7,hp:7,speed:132}}[type];
...
function spawnEnemy(){
  ...
  const roll=Math.random();let type='melee';if(roll<.12)type='swarm';else if(elapsed>15&&roll<.24)type='exploder';else if(elapsed>22&&roll<.36)type='shooter';else if(elapsed>35&&roll<.47)type='charger';else if(roll>.93)type='brute';
