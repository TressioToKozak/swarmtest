...
function blocked(x,y,r=14){
  ...
  return typeof scenery!=='undefined'&&scenery.some(s=>s.type==='rock'&&Math.hypot(x-s.x,y-s.y)<r+s.size*.72);
}
function projectileBlocked(x,y){return terrain.some(t=>t.type==='wall'?circleRectCollision(x,y,2,t):t.type==='rock'&&Math.hypot(x-t.x,y-t.y)<t.r*.94)||(typeof scenery!=='undefined'&&scenery.some(s=>s.type==='rock'&&Math.hypot(x-s.x,y-s.y)<s.size*.72))}
...
document.querySelector('#startBtn').onclick=start;document.querySelector('#continueBtn').onclick=continueGame;document.querySelector('#restartBtn').onclick=()=>location.reload();document.querySelector('#instructionsBtn').onclick=()=>{ui.startModal.classList.add('hidden');ui.instructionsModal.classList.remove('hidden')};document.querySelector('#backBtn').onclick=()=>{ui.instructionsModal.classList.add('hidden');ui.startModal.classList.remove('hidden')};document.querySelector('#statsBtn').onclick=showStats;document.querySelector('#charactersBtn').onclick=()=>openMenuPanel(ui.charactersModal);document.querySelector('#mapsBtn').onclick=()=>openMenuPanel(ui.mapsModal);document.querySelector('#charactersBackBtn').onclick=()=>closeMenuPanel(ui.charactersModal);document.querySelector('#mapsBackBtn').onclick=()=>closeMenuPanel(ui.mapsModal);document.querySelector('#statsBackBtn').onclick=()=>{ui.statsModal.classList.add('hidden');ui.startModal.classList.remove('hidden')};document.querySelector('#pauseBtn').onclick=togglePause;document.querySelector('#resumeBtn').onclick=togglePause;document.querySelector('#quitBtn').onclick=gameOver;
function openMenuPanel(panel){ui.startModal.classList.add('hidden');panel.classList.remove('hidden')}function closeMenuPanel(panel){panel.classList.add('hidden');ui.startModal.classList.remove('hidden')}
...
function openCrate(crate){
  crate.opened=true;const roll=Math.random();if(roll<.4){player.hp=Math.min(player.maxHp,player.hp+35);crate.reward='health';burst(crate.x,crate.y,'#ff6688',18)}else if(roll<.8){crate.reward='xp';gainXp(15+Math.floor(player.level*2));burst(crate.x,crate.y,'#51f6df',18)}else{crate.reward='level';gainXp(player.nextXp-player.xp);burst(crate.x,crate.y,'#ffd467',24)}
}
...
function draw(){ctx.save();if(screenShake)ctx.translate((Math.random()-.5)*screenShake,(Math.random()-.5)*screenShake);ctx.fillStyle='#070b15';ctx.fillRect(0,0,W,H);ctx.translate(-camera.x,-camera.y);
  ...
  crates.forEach(drawCrate);orbs.forEach(o=>{ctx.shadowBlur=16;ctx.shadowColor='#51f6df';ctx.fillStyle='#80fff0';ctx.beginPath();ctx.moveTo(o.x,o.y-o.r);ctx.lineTo(o.x+o.r,o.y);ctx.lineTo(o.x,o.y+o.r);ctx.lineTo(o.x-o.r,o.y);ctx.fill();ctx.shadowBlur=0});
