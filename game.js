...
const ui = Object.fromEntries(['timer','kills','level','healthText','healthBar','xpLevel','xpText','xpBar','slots','cards','startModal','instructionsModal','statsModal','pauseModal','levelModal','endModal','newLevel','endStats','bestTime','totalTime','bestKills','totalKills','phaseLabel','waveBanner','waveCaption','waveTitle','charactersModal','mapsModal','endEyebrow','endTitle'].map(id=>[id,document.getElementById(id)]));
let W=0,H=0,dpr=1,last=0,running=false,paused=true,elapsed=0,spawnClock=0,shotClock=0,screenShake=0,flowClock=0,statsClock=0,saveClock=0,directorPhase=-1,waveNumber=0,waveRemaining=0,waveBreak=0,bannerTimer=0,bossTierSpawned=0,levelChoicesQueued=0;
...
function saveGame(){
  if(!running)return;const state={version:2,savedAt:Date.now(),elapsed,spawnClock,shotClock,directorPhase,waveNumber,waveRemaining,waveBreak,bossTierSpawned,levelChoicesQueued,player:{...player},skills:{q:{...skills.q},e:{...skills.e},r:{...skills.r}},levels:{...levels},enemies:enemies.slice(-450),bullets:bullets.slice(-250),enemyBullets:enemyBullets.slice(-250),orbs:orbs.slice(-350),loot,crates,rings,levelPending:!ui.levelModal.classList.contains('hidden')};
...
function rewardBoss(tier){if(tier===3){winMap();return}for(let i=0;i<tier;i++){player.level++;player.nextXp=Math.floor(player.nextXp*1.34);levelChoicesQueued++}if(!paused)showLevelUp()}
