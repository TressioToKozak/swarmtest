(() => {
  let session=null,state=null,previous=null,receivedAt=0,sendClock=0,lastLevelRound=0,lastWaveSeq=0;
  let remotePlayers=[],effectiveSpeed=0,remoteVisuals=new Map(),localAttackSeq=0,localAbilitySeq=0;
  let previousEnemies=new Map(),previousBullets=new Map();
  ...
  function accept(message){
    ...
    if(message.type==='gameStarted'){
      ...
      remoteVisuals.clear();remotePlayers=[];previousEnemies.clear();previousBullets.clear();localAttackSeq=0;localAbilitySeq=0;
  ...
  function applyState(){
    if(!state)return;
    elapsed=state.time;player.level=state.level;player.xp=state.xp;player.nextXp=state.nextXp;paused=state.paused;
    if(!paused)document.getElementById('multiplayerWaiting').classList.add('hidden');
    ...
    if(mine){
      const error=Math.hypot(player.x-mine.x,player.y-mine.y);
      if(error>90){player.x=mine.x;player.y=mine.y}else{player.x+=(mine.x-player.x)*.2;player.y+=(mine.y-player.y)*.2}
      ...
      if(mine.cooldowns){skills.q.cd=mine.cooldowns.q;skills.e.cd=mine.cooldowns.e;skills.r.cd=mine.cooldowns.r}
    }
    previousEnemies=new Map((previous?.enemies||[]).map(e=>[e.id,e]));previousBullets=new Map((previous?.bullets||[]).map(b=>[b.id,b]));
  ...
  function frame(dt){
    ...
    if(sendClock<=0){const aim=Math.atan2(mouse.y+camera.y-player.y,mouse.x+camera.x-player.x);socket().send('playerInput',{up:!!keys.w,down:!!keys.s,left:!!keys.a,right:!!keys.d,aim,q:!!keys.q,e:!!keys.e,r:!!keys.r});sendClock=1/25}
    ...
    if(state){const now=performance.now(),alpha=Math.min(1,(now-receivedAt)/72);remotePlayers=[...remoteVisuals.values()].map(visual=>SwarmRemoteVisuals.sample(visual,now,72));enemies=state.enemies.map(current=>{const old=previousEnemies.get(current.id)||current;return{...current,x:old.x+(current.x-old.x)*alpha,y:old.y+(current.y-old.y)*alpha,attackCd:1,chargeCd:1,phase:0,hit:SwarmRemoteVisuals.damageFlash(old,current)}});bullets=state.bullets.map(current=>{const old=previousBullets.get(current.id)||current;return{...current,x:old.x+(current.x-old.x)*alpha,y:old.y+(current.y-old.y)*alpha,r:current.r||4,kind:current.kind||'scoutBullet'}})}
  }
  function levelShown(){}
