(() => {
  let session=null,state=null,previous=null,receivedAt=0,sendClock=0,lastLevelRound=0,remotePlayers=[],effectiveSpeed=0;
  ...
  function accept(message){
    ...
    if(message.type==='gameStarted'){state=null;previous=null;lastLevelRound=0}
  ...
  function applyState(){
    if(!state)return;elapsed=state.time;player.level=state.level;player.xp=state.xp;player.nextXp=state.nextXp;paused=state.paused;const mine=state.players.find(p=>p.id===session.playerId);
    if(mine){const error=Math.hypot(player.x-mine.x,player.y-mine.y);if(error>90){player.x=mine.x;player.y=mine.y}else{player.x+=(mine.x-player.x)*.35;player.y+=(mine.y-player.y)*.35}player.hp=mine.hp;player.maxHp=mine.maxHp;player.kills=mine.kills;effectiveSpeed=mine.speed||characterDefs[chosenCharacter].speed;if(mine.cooldowns){skills.q.cd=mine.cooldowns.q;skills.e.cd=mine.cooldowns.e;skills.r.cd=mine.cooldowns.r}}
    remotePlayers=state.players.filter(p=>p.id!==session.playerId);enemies=state.enemies.map(e=>({...e,type:e.type||'melee',hit:0,attackCd:1,chargeCd:1,phase:0}));bullets=state.bullets.map(b=>({...b,r:b.r||4,kind:b.kind||'scoutBullet'}));enemyBullets=(state.enemyBullets||[]).map(b=>({...b,r:b.r||6}));orbs=(state.orbs||[]).map(o=>({...o,r:5,vx:0,vy:0}));loot=state.loot||[];crates=state.crates||crates;camera.x=Math.max(0,Math.min(WORLD.w-W,player.x-W/2));camera.y=Math.max(0,Math.min(WORLD.h-H,player.y-H/2));updateUI()
  }
  function frame(dt){
    if(!isActive())return;sendClock-=dt;if(sendClock<=0){const aim=Math.atan2(mouse.y+camera.y-player.y,mouse.x+camera.x-player.x);socket().send('playerInput',{up:!!keys.w,down:!!keys.s,left:!!keys.a,right:!!keys.d,aim,q:!!keys.q,e:!!keys.e,r:!!keys.r});sendClock=1/25}
    ...
    if(state){const alpha=Math.min(1,(performance.now()-receivedAt)/80);remotePlayers=state.players.filter(p=>p.id!==session.playerId).map(current=>{const old=previous?.players.find(p=>p.id===current.id)||current;return{...current,x:old.x+(current.x-old.x)*alpha,y:old.y+(current.y-old.y)*alpha}});enemies=state.enemies.map(current=>{const old=previous?.enemies.find(e=>e.id===current.id)||current;return{...current,x:old.x+(current.x-old.x)*alpha,y:old.y+(current.y-old.y)*alpha,hit:0,attackCd:1,chargeCd:1,phase:0}});bullets=state.bullets.map(current=>{const old=previous?.bullets.find(b=>b.id===current.id)||current;return{...current,x:old.x+(current.x-old.x)*alpha,y:old.y+(current.y-old.y)*alpha,r:current.r||4}})}
  }
  function levelShown(){}
