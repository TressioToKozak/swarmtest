(() => {
  ...
  function accept(message){
    ...
    if(message.type==='gameStarted'){state=null;previous=null;lastLevelRound=0;remoteVisuals.clear();remotePlayers=[];localAttackSeq=0;localAbilitySeq=0}
  ...
  function applyState(){
    ...
    remotePlayers=state.players.filter(p=>p.id!==session.playerId);SwarmRemoteVisuals.update(remoteVisuals,remotePlayers,receivedAt);enemies=state.enemies.map(e=>({...e,type:e.type||'melee',hit:0,attackCd:1,chargeCd:1,phase:0}));bullets=state.bullets.map(b=>({...b,r:b.r||4,kind:b.kind||'scoutBullet'}));enemyBullets=(state.enemyBullets||[]).map(b=>({...b,r:b.r||6}));orbs=(state.orbs||[]).map(o=>({...o,r:5,vx:0,vy:0}));loot=state.loot||[];crates=state.crates||crates;camera.x=Math.max(0,Math.min(WORLD.w-W,player.x-W/2));camera.y=Math.max(0,Math.min(WORLD.h-H,player.y-H/2));updateUI()
  }
  function frame(dt){
