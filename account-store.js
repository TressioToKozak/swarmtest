...
const SERVER_PROGRESS_KEYS = new Set([
  "swarmfall-stats",
  "swarmfall-achievements-v1",
  "swarmfall-unlocked",
  "swarmfall-modes",
...
const CHARACTER_COSTS = require("./shared-game-data").characterCosts;
const MODE_IDS = new Set(["normal", "hard", "nightmare", "endless"]);
...
function mergeClientProgress(current, incoming) {
  const next = cleanProgress(incoming);
...
function applyProgressOperation(progress, operation) {
  ...
  if (type === "awardCurrency") {
  ...
  } else if (type === "awardSingleplayerResult") {
    const time = Math.floor(payload.time),
      kills = Math.floor(payload.kills),
      character = String(payload.character || "scout");
    if (
      ...
      kills < 0 ||
      kills > 100000 ||
      !["scout", "warrior", "druid"].includes(character)
    )
      throw progressionError("INVALID_PROGRESSION");
    stats.bestTime = Math.max(stats.bestTime, time);
    ...
    if (!achievements.includes(id)) {
      ...
      stats.coins = Math.max(0, stats.coins) + (ACHIEVEMENT_REWARDS[id] || 0);
      progress["swarmfall-achievements-v1"] = JSON.stringify(achievements);
      writeStats(progress, stats);
    }
  } else if (type === "purchaseCharacter") {
    const character = String(payload.character || ""),
      cost = CHARACTER_COSTS[character];
    if (!cost) throw progressionError("INVALID_PROGRESSION");
    if (!unlocked.includes(character)) {
      if (stats.coins < cost) throw progressionError("INSUFFICIENT_COINS");
      stats.coins -= cost;
      unlocked.push(character);
      progress["swarmfall-unlocked"] = JSON.stringify(unlocked);
      writeStats(progress, stats);
    }
  } else if (type === "completeMap") {
    const map = String(payload.map || ""),
      mode = String(payload.mode || "");
    if (
      !["ruins", "toxic"].includes(map) ||
      !MODE_IDS.has(mode) ||
      (map === "toxic" && !achievements.includes("map_1")) ||
      (mode === "hard" && !modes.includes("normal")) ||
      (mode === "nightmare" && !modes.includes("hard")) ||
      (mode === "endless" && !modes.includes("normal"))
    )
      throw progressionError("INVALID_PROGRESSION");
    ...
    for (const id of earned)
      if (!achievements.includes(id)) {
        achievements.push(id);
        stats.coins += ACHIEVEMENT_REWARDS[id];
      }
    if (earned.length) {
      progress["swarmfall-achievements-v1"] = JSON.stringify(achievements);
      writeStats(progress, stats);
...
function mergeMultiplayerStats(raw, result) {
  ...
  const stats = {
      ...
      ...parsed,
      characterGames: {
        ...DEFAULT_STATS.characterGames,
        ...parsed.characterGames,
      },
    },
...
class AccountStore {
  ...
  applySingleplayerOperation(userId, expectedRevision, operation) {
    return this.mutate((data) => {
      ...
      user.lastRevisionKind = "singleplayer-operation";
      return {
        saved: true,
